...
Rule này áp dụng cho mọi task, mọi thay đổi code và mọi commit trong repository đang làm việc.
...
- Mô hình dữ liệu (data model), schema và chiến lược lưu trữ.
- Các pipeline xử lý, chuyển đổi hoặc nhập dữ liệu.
...
Hướng dẫn dành cho người dùng cuối. Nội dung phải phản ánh hành vi thực tế của ứng dụng và bao gồm, khi có:

- Mục đích và phạm vi của ứng dụng.
- Cách bắt đầu sử dụng và điều hướng trong ứng dụng.
- Cách dùng từng tính năng chính: mục đích, các bước thao tác và kết quả mong đợi.
- Giải thích các thuật ngữ chuyên ngành được sử dụng trong ứng dụng.
- Các lỗi thường gặp và cách xử lý.

Với thư viện, CLI hoặc service, người dùng cuối là người gọi API, chạy CLI hoặc tích hợp service đó.
...
2. Thêm tính năng mới hoặc thay đổi hành vi mà người dùng nhìn thấy hoặc tương tác phải cập nhật `USER_GUIDE.md`.
3. Thay đổi kiến trúc, mô hình dữ liệu, pipeline dữ liệu, state management, cấu trúc thư mục, command, script hoặc dependency phải cập nhật `DEVELOPMENT.md`.
4. Hoàn thành task phải cập nhật cả `CHANGELOG.md` và `ROADMAP.md`.
5. Tài liệu phải đủ rõ để người khác clone repository, cài đặt, chạy ứng dụng, kiểm tra và tiếp tục phát triển.
6. Chỉ ghi thông tin đã được xác minh từ code, cấu hình, kết quả chạy command hoặc yêu cầu đã được xác nhận.
7. Không ghi thông tin giả hoặc suy đoán như sự thật. Phần chưa được triển khai hoặc chưa xác minh phải ghi rõ `Chưa có` hoặc `Sẽ bổ sung sau`.
8. Không ghi secret, token, private key, mật khẩu hoặc thông tin tài khoản thật vào bất kỳ tài liệu nào. Không sao chép nguyên văn nội dung có bản quyền hoặc bị giới hạn giấy phép của bên thứ ba vào tài liệu.
9. Khi tài liệu mâu thuẫn với code, phải coi code và kết quả kiểm tra thực tế là nguồn để xác minh, sau đó sửa tài liệu trong cùng task.
10. Không xóa thông tin lịch sử hợp lệ khỏi `CHANGELOG.md`; chỉ sửa khi thông tin đó sai hoặc gây hiểu nhầm.
...
2. Test, lint và build (những gì dự án có) đã được chạy; nếu không chạy được, phải ghi rõ command chưa chạy, lý do và ảnh hưởng dự kiến.
